Public Class BuscarDocumentoAplicado
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If datosDocumento Is Nothing Then
            respuesta = New With {
                .estado = False,
                .lista = {},
                .mensaje = "No se logró consultar el documento seleccionado."
            }
        Else
            If datosDocumento.idProveedor > 0 AndAlso datosDocumento.Moneda IsNot Nothing Then
                ...
                Dim nombreTipoDocumento = ObtenerNombreTipoDocumento(tipoDocumento, errorMessage)
                respuesta = New With {
                .estado = True,
                .documento = FormatearRespuestaDocumento(idCategoriaDocumento, nombreProveedor, nombreTipoDocumento, datosDocumento),
                .mensaje = ""
            }
            Else
                respuesta = New With {
                .estado = False,
                .documento = {},
                .mensaje = "No se encontró información del documento seleccionado."
            }
            End If
        End If
    ...
    End Function
    Private Function ObtenerNombreTipoDocumento(idTipoDoc As Integer, ByRef errorMessage As String) As String
        Dim objTipoDocumentoDB As New TipoDocumentoDB
        Dim modTipoDocumento As New Models.TipoDocumento

        modTipoDocumento = objTipoDocumentoDB.BuscaTipoDocumento_x_IDTipo(idTipoDoc, errorMessage)
        Return modTipoDocumento.Descripcion
    End Function
    ...
    Private Function FormatearRespuestaDocumento(idCategoria As Integer, nombreProveedor As String, nombreTipoDocumento As String, documento As Object) As RespuestaDocumento
        Dim datosDocumento As New RespuestaDocumento()
        Dim objHerramientas As New Herramientas
        If idCategoria = 1 Then
            datosDocumento.IDCategoria = idCategoria
            datosDocumento.NombreProveedor = nombreProveedor
            datosDocumento.TipoDocumento = nombreTipoDocumento
            datosDocumento.NumDocumento = documento.NumeroFactura
            datosDocumento.Observacion = documento.Observacion
            datosDocumento.FechaDoc = CDate(documento.FechaEmision).ToString("dd-MM-yyyy").Replace("-", "/")
            datosDocumento.MonedaDoc = $"{documento.Moneda} {documento.Simbolo}"
            datosDocumento.MontoTotal = $"{documento.Simbolo}{objHerramientas.FormatearMonto(documento.Total)}"
